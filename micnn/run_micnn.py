...
import importlib
import sys
import micnn.micnn_utilities as network
import data.micnn as data
...
def create_logdir(method, weight, rd):
    ...
    return "micnn/train_logs/kdd/{}/{}/{}".format(weight, method, rd)
...
def train_and_test(nb_epochs, weight, method, degree, random_seed):
    ...
    with sv.managed_session() as sess:
        ...
        y_pred[inds_comp] = 1
        # 计算精度，召回率，f1分数
        precision, recall, t, _ = precision_recall_fscore_support(testy,
                                                                   y_pred,
                                                                   average='binary')
        f1 = t
        print(
